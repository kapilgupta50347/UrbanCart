  ...
  return (
    ...
    >
      <Link to={`/product/${product.id}`}>
        <div className="rounded-lg border bg-card text-card-foreground shadow-sm glass-card border-white/10 text-white overflow-hidden group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 h-full flex flex-col">
          <div className="relative">
            ...
            {product.ribbon_text && (
              <div className="absolute top-3 left-3 bg-[#FFD23F]/90 text-[#001F3F] text-xs font-bold px-3 py-1 rounded-full shadow-lg">
                {product.ribbon_text}
              </div>
            )}
            <div className="absolute top-3 right-3 bg-blue-500/80 text-white text-xs font-bold px-3 py-1 rounded-full flex items-baseline gap-1.5">
              {hasSale && (
          ...
          <div className="p-4 flex flex-col flex-grow">
            ...
            {product.subtitle && <p className="text-sm text-gray-300 h-10 overflow-hidden">{product.subtitle}</p>}
            <div className="mt-2 flex items-center text-sm text-[#FFD23F]">
...
const ProductsList = () => {
  ...
  const getSellerForProduct = (productId) => {
    // Simple logic to assign a seller based on product ID
  ...
  useEffect(() => {
    const fetchProductsWithQuantities = async () => {
      try {
        ...
        const productsWithDetails = productsResponse.products.map(product => ({
          ...product,
          seller: getSellerForProduct(product.id),
